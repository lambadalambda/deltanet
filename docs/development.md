# Headwater development and operations
...
This document contains the source-build, deployment, security, testing, and
compatibility reference for contributors and self-hosting operators. Commands
and repository paths are relative to the repository root unless stated
otherwise. For the user-facing introduction, see the [project README](../README.md).
...
The daemon, frontend, and desktop packages intentionally keep separate
dependency lockfiles because they have independent dependency graphs and build
contexts. The empty root
importer lockfile is also intentional: pnpm's root script lifecycle maintains it
even though dependencies are installed only in the three packages. Root scripts
...
`.github/workflows/nightly.yml` runs the default daemon unit, frontend browser,
and desktop test suites plus workspace checks. It packages Linux x64
Flatpak/AppImage, Windows x64 NSIS, and macOS arm64 DMG artifacts on native
runners, generates `SHA256SUMS.txt`, and replaces the rolling
[`nightly` prerelease](https://github.com/lambadalambda/headwater/releases/tag/nightly)
only when all jobs pass and the built commit is still the tip of `main`.
Nightlies are intentionally unsigned, manually updated development builds; the
workflow's release notes are the canonical platform installation and data-path
reference.
...
- `desktop/` — Electron host, native packaging configuration, bundled-resource
  staging, and desktop lifecycle/smoke tests.
...
history. Those paths are from the repository root.
...
Some existing checkout directories may still use the former repository slug;
the canonical remote is now `lambadalambda/headwater`.
